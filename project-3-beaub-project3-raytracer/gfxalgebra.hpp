...
public:
  ...
  constexpr bool approx_equal(const same_type& other, scalar_type delta) const noexcept {
    for (size_t i = 0; i < elements_.size(); i++) {
      scalar_type lowerBound = other[i] - delta;
      scalar_type upperBound = other[i] + delta;
      if (elements_[i] < lowerBound || elements_[i] > upperBound) {
        return false;
      }
    }
    return true;
  ...
  grow(scalar_type default_value = 0) const noexcept {
    ...
    size_t lastIndex;
    for (lastIndex = 0; lastIndex < elements_.size(); lastIndex++) {
      result[lastIndex] = elements_[lastIndex];
    }
    for (; lastIndex < NEW_DIMENSION; lastIndex++) {
      result[lastIndex] = default_value;
    }
    return result;
  }
  ...
  bool is_unit(scalar_type delta) const noexcept {
    ...
    scalar_type lowerBound = 1 - delta;
    scalar_type upperBound = 1 + delta;
    for (auto value : elements_) { length += (value * value); }
    length = sqrt(length);
    if (length <= lowerBound || length >= upperBound) { return false; }
    return true;
  }

  // Return true if every element of this vector is == 0.
  ...
  constexpr scalar_type magnitude_squared() const noexcept {
    scalar_type magnitudeSquared = 0;
    for (auto value : elements_) { magnitudeSquared += value * value; }
  ...
  same_type normalized() const noexcept {
    same_type result;
    for (size_t i = 0; i < elements_.size(); i++) { result[i] = elements_[i] / magnitude(); }
    return result;
  }
};
...
public:
  ...
  constexpr same_type operator+ (const same_type& rhs) const noexcept {
    ...
    for (size_t row = 0; row < HEIGHT; row++) {
      for (size_t col = 0; col < WIDTH; col++) {
        result[row][col] = rows_[row][col] + rhs[row][col];
      }
    }
    return result;
  }
  ...
  constexpr same_type operator- () const noexcept {
    ...
    for (size_t row = 0; row < HEIGHT; row++) {
      for (size_t col = 0; col < WIDTH; col++) {
        result[row][col] = rows_[row][col]*-1;
      }
    }
    return result;
  }
  ...
  constexpr same_type operator* (const scalar_type rhs) const noexcept {
    ...
    for (size_t row = 0; row < HEIGHT; row++) {
      for (size_t col = 0; col < WIDTH; col++) {
        result[row][col] = rows_[row][col] * rhs;
      }
    }
    return result;
  }
  ...
  constexpr bool approx_equal(const same_type& other, scalar_type delta) const noexcept {
    for (size_t row = 0; row < HEIGHT; row++) {
      for (size_t col = 0; col < WIDTH; col++) {
        scalar_type currThisElem = rows_[row][col];
        scalar_type currOtherElem = other[row][col];
        scalar_type lowerBound = currOtherElem - delta;
        scalar_type upperBound = currOtherElem + delta;
        if (currThisElem < lowerBound || currThisElem > upperBound) {
          return false;
        }
      }
    }
  ...
  submatrix(size_t top_row = 0, size_t left_column = 0) const noexcept {
    ...
    size_t i = 0, j = 0;
    for (size_t currentRow = top_row; i < NEW_HEIGHT; currentRow++, i++) {
      for (size_t currentCol = left_column; j < NEW_WIDTH; currentCol++, j++) {
        result[i][j] = rows_[currentRow][currentCol];
      }
      j = 0;
    }
    return result;
  ...
  scalar_type determinant() const noexcept {
    ...
    scalar_type d;
    if (WIDTH == 2) {
      d = (rows_[0][0] * rows_[1][1]) - (rows_[0][1] * rows_[1][0]);
    }
    else { // width = 3
      scalar_type detA = rows_[0][0]*((rows_[1][1] * rows_[2][2]) - (rows_[1][2] * rows_[2][1]));
      scalar_type detB = rows_[0][1]*((rows_[1][0] * rows_[2][2]) - (rows_[1][2] * rows_[2][0]));
      scalar_type detC = rows_[0][2]*((rows_[1][0] * rows_[2][1]) - (rows_[1][1] * rows_[2][0]));
      d = detA - detB + detC;
    }
    return d;
  }
  ...
  vector<scalar_type, HEIGHT> solve(const vector<scalar_type, HEIGHT>& b) const noexcept {
    ...
    vector<scalar_type, HEIGHT> result;
    if (WIDTH == 2) {
      scalar_type detCoefficientMatrix = (rows_[0][0] * rows_[1][1]) - (rows_[0][1] * rows_[1][0]);

      matrix<scalar_type,2,2> xMatrix = { b[0], rows_[0][1], b[1], rows_[1][1] };
      scalar_type detXMatrix = xMatrix.determinant();

      matrix<scalar_type,2,2> yMatrix = { rows_[0][0], b[0], rows_[1][0], b[1] };
      scalar_type detYMatrix = yMatrix.determinant();

      result[0] = detXMatrix / detCoefficientMatrix;
      result[1] = detYMatrix / detCoefficientMatrix;
    }
    else {  // 3x3
      scalar_type detA = rows_[0][0]*((rows_[1][1] * rows_[2][2]) - (rows_[1][2] * rows_[2][1]));
      scalar_type detB = rows_[0][1]*((rows_[1][0] * rows_[2][2]) - (rows_[1][2] * rows_[2][0]));
      scalar_type detC = rows_[0][2]*((rows_[1][0] * rows_[2][1]) - (rows_[1][1] * rows_[2][0]));
      scalar_type detCoefficientMatrix = detA - detB + detC;

      matrix<scalar_type,3,3> xMatrix = { b[0], rows_[0][1], rows_[0][2],
                                          b[1], rows_[1][1], rows_[1][2],
                                          b[2], rows_[2][1], rows_[2][2] };
      scalar_type detXMatrix = xMatrix.determinant();

      matrix<scalar_type,3,3> yMatrix = { rows_[0][0], b[0], rows_[0][2],
                                          rows_[1][0], b[1], rows_[1][2],
                                          rows_[2][0], b[2], rows_[2][2] };
      scalar_type detYMatrix = yMatrix.determinant();

      matrix<scalar_type,3,3> zMatrix = { rows_[0][0], rows_[0][1], b[0],
                                          rows_[1][0], rows_[1][1], b[1],
                                          rows_[2][0], rows_[2][1], b[2] };
      scalar_type detZMatrix = zMatrix.determinant();
      result[0] = detXMatrix / detCoefficientMatrix;
      result[1] = detYMatrix / detCoefficientMatrix;
      result[2] = detZMatrix / detCoefficientMatrix;
    }
    return result;
  }
  ...
  static constexpr same_type identity() noexcept {
    same_type result;
    for (size_t i = 0; i < HEIGHT; i++) { result[i][i] = 1; }
    return result;
  }
